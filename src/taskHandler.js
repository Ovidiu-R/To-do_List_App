...
export const filterByDate = (taskArray, filterId) => {
  ...
  var normalisedCurrentDate = new Date(
  ...
  if (filterId === "day") {
    const dailyTasks = taskArray.filter((task) => {
      ...
      const normalisedTaskDeadline = new Date(
        taskDeadline.getFullYear(),
        taskDeadline.getMonth(),
        taskDeadline.getDate()
      );
      return (
        normalisedTaskDeadline.getFullYear() ===
          normalisedCurrentDate.getFullYear() &&
        normalisedTaskDeadline.getMonth() ===
          normalisedCurrentDate.getMonth() &&
...
export const positiveProjectCounter = (activeProjectId) => {
  ...
  return projectArray.some(
    (project) => project.name === activeProjectId && project.counter > 0
  ); // the some() method returns true if any of the array elements match the specified condition
};
...
export const deleteProject = () => {
  ...
  const updatedProjectArray = projectArray.filter(
    (project) => project.name !== activeProject.id
  );
  ...
  switch (true) {
    ...
    case positiveProjectCounter(taskToDelete.project):
      ...
      if (activeDateFilter === null) {
        ...
        displayTasks(
          sortByDate(filterByDate(fetchTasks(), activeDateFilter.id))
        );
      }
      break;
    case true:
